...
{
	hint "You bring a vehicle, belonging to you or an other policemen. This will cost $500!";
	
	sleep 1.5;
	
	if(life_atmcash - 500 < 0) exitWith
	{
		hint "You have insufficient funds in your bank account!";
	};
};
...
if((_vehicle isKindOf "Car") || (_vehicle isKindOf "Air") || (_vehicle isKindOf "Ship")) then
{
	...
	if(!_costful_impound) then
	{
		[[0,format["%1 your %2 is seized by the police.",(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	};
	...
	if(player distance _vehicle > 10) exitWith {hint "Canceled seizures."; life_action_inUse = false;};
	...
	if((count crew _vehicle) == 0) then
	{
		...
		else
		{
			...
			[[0,format["%1 has seized the vehicle of %2 (vehicle: % 3)",name player,(_vehicleData select 0) select 1,_vehicleName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
