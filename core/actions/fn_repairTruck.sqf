...
if((_veh isKindOf "Car") OR (_veh isKindOf "Ship") OR (_veh isKindOf "Air")) then
{
	...
	if(_damage > 0.5) exitWith
	{
		...
		hint "the vehicle is damaged too much! You can not fix it! But you can call the ADAC. ";
	};
	...
	if("ToolKit" in (items player)) then
	{
		...
		if(player != vehicle player) exitWith {titleText["You need the vehicle out, to repair it!", "PLAIN"];};
