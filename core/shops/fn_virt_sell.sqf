...
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "You have not so many items for sale!"};
...
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	...
	{
		hint format["You're selling %1 %2 for $%3. You gotta go only to launder the money, to be able to use it.",_amount,_name,[_price] call life_fnc_numberText];
		[true,"illegalmoney",_price] call life_fnc_handleInv;
	}
	else
	{
		hint format["You're selling %1 %2 for $%3",_amount,_name,[_price] call life_fnc_numberText];
