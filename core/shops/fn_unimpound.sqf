...
if(_veh distance impound_obj < 50) then
{
	...
	if(count _check > 0) exitWith {hint "There currently is a vehicle.";};
	if(_price > life_cash) then
	{
		_price = _price + 200;
		if(_price > life_atmcash) exitWith {hint "You have not enough money in your bank account to your vehicle back to get."};
	...
	hint format["You've got your %1 for $%3 parked out.",_name,[_price] call life_fnc_numberText];
	detach _veh;
