...
switch(life_veh_shop) do
{
	...
	case "cop_air_5":
	{
		_sp = getMarkerPos "cop_air_1_1_1";
		_dir = markerDir "cop_air_1_1_1";
	};
...
if(!([_veh] call life_fnc_vehShopLicenses)) exitWith {hint "You have not the required license!"};
...
if(life_cash < _price) exitWith {hint format["You have not enough money. You need %1", [_price] call life_fnc_numberText]; };
hint "Can a two seconds...";
sleep floor(random 3);

if(count(nearestObjects[_sp,["Car","Ship","Air"],4]) > 0) exitWith {hint "A vehicle is on the spawn."};
