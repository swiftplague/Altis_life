...
*/
private["_unit","_shooter","_curWep","_curMags","_attach","_impact"];
_unit = [_this,0,Objnull,[Objnull]] call BIS_fnc_param;
_shooter = [_this,1,Objnull,[Objnull]] call BIS_fnc_param;
_impact = [_this, 2, 15, [0]] call BIS_fnc_param; //Impact value for different weapons
if(isNull _unit OR isNull _shooter) exitWith {player allowDamage true; life_istazed = false;};

if(_impact <= 0) then {_impact = 15; };
...
if(_shooter isKindOf "Man" && alive player) then
{
	if(!life_istazed) then
	{
		...
		//event stuff
		//send to event function
		if( [] call life_fnc_playerInEvent ) then
		{
			[] spawn life_fnc_playerEventTazed;
		};
