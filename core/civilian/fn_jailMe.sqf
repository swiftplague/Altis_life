...
if(_time <= 0) then { _time = time + (15 * 60); hintC "Please login Admin: JAIL_FALLBACK_15, time is zero!"; };
...
while {true} do
{
	if((round(_time - time)) > 0) then
	{
		...
		{
			hintSilent format["Time remaining:\n %1\n\nYou must pay the security deposit\nSecurity deposit: $%2",_countDown,[life_bail_amount] call life_fnc_numberText];
		}
		else
		{
...
switch (true) do
{
	case (_bail) :
	{
		...
		hint "You have paid the deposit and now free.";
	...
	{
		life_is_arrested = false;
		hint "You're broke out of the prison. This is a further offence!";
	...
	{
		life_is_arrested = false;
		hint "You've served your time and are now free.";
		[[getPlayerUID player],"life_fnc_wantedRemove",false,false] spawn life_fnc_MP;
		player setPos (getMarkerPos "jail_release");
		[1,false] call life_fnc_sessionHandle;
	};
