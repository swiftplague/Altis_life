...
if(player distance _vault > 10) exitWith {[[_vault,-1],"TON_fnc_robberyState",false,false] spawn life_fnc_MP; hint "You have to stay in the vicinity of the safe!"};

if(_funds < 50000) exitWith{[[_vault,-1],"TON_fnc_robberyState",false,false] spawn life_fnc_MP; hint "The Bank has not enough credits...";};

_timer = time + (10 * 60); //Default timer is 10 minutes to rob.
titleText["Crack the safe ...","PLAIN"];
[[2,"$$$ THE STATE BANK IS ROBBED!!! $$$"],"life_fnc_broadcast",nil,false] spawn life_fnc_MP;
...
while {true} do
{
	...
	hintSilent format["You have to keep distance up to 15 m!\n\nRemaining time:\n %1\n\nDistance: %2m",_countDown,round(player distance _vault)];
...
switch(true) do
{
	case (_toFar):
	{
		hint "You have yourself too far away from the safe! Failed bank robbery!";
		[[_vault,0],"TON_fnc_robberyState",false,false] spawn life_fnc_MP;
	};
	
	case (!alive player):
	{
		hint "You're dead! Failed bank robbery!";
		[[_vault,0],"TON_fnc_robberyState",false,false] spawn life_fnc_MP;
	};
	
	case (life_istazed):
	{
		hint "You were stunned! Failed bank robbery!";
		[[_vault,0],"TON_fnc_robberyState",false,false] spawn life_fnc_MP;
	};
	
	case ((round(_timer - time)) < 1):
	{
		hint format["You have successfully $%1 robbed!\n\nTo prevent tracing, you can use for 8 min no ATM.",[_funds] call life_fnc_numberText];
		[] spawn
		{
			life_use_atm = false;
			sleep 480;
