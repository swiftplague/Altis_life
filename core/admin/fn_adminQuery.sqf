...
if(!isNil {admin_query_ip}) exitWith {hint "You're asking a player already off."};
_text = _display displayCtrl 2903;
_info = lbData[2902,lbCurSel (2902)];
_info = call compile format["%1", _info];
if(isNil "_info") exitWith {_text ctrlSetText "Players no longer exists?";};
if(isNull _info) exitWith {_text ctrlSetText "Players no longer exists?";};
[[player],"fnc_player_query",_info,false] spawn life_fnc_MP;
_text ctrlSetText "Get player...";