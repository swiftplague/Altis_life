...
{
	systemChat "you used a Perm, which is not currently loaded. Please wait. ";
};
...
{
	[[0,format[">PERM< Player %1 wanted to access missing Permission %2! PLEASE REPORT TO ADMIN!", name player, _permid]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};
...
if(!_valid) exitWith
{
	[[0,format[">PERM< Perm for players could not set %1 of the group of '%2' to %3!", name player, _perm_name, _level]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};
...
switch(true) do 
{
	...
	case (_level == 0):
	{
		_message = format["<img size='10' color='#FFFFFF' image='%1'/><br/><br/><t size='2.5'>%2</t><br/><t size='1.5'>Thrown out!</t><br/><t size='1'>You're now a member of the group more!</t>", _perm_logo,_perm_name];
		hint parseText _message;
	};
	case (_oldlevel == 0):
	{
		_message = format["<img size='10' color='#FFFFFF' image='%1'/><br/><br/><t size='2.5'>%2</t><br/><t size='1.5'>Level %3</t><br/><t size='1'>You were included in the Group!</t>", _perm_logo,_perm_name, _level];
		hint parseText _message;
	};
	case (_level > _oldlevel):
	{
		_message = format["<img size='10' color='#FFFFFF' image='%1'/><br/><br/><t size='2.5'>%2</t><br/><t size='1.5'>Level %3</t><br/><t size='1'>You've got a h higher rank!</t>", _perm_logo,_perm_name, _level];
		hint parseText _message;
	};
	case (_level < _oldlevel):
	{
...
if(_adminmode) then
{
	[[0,format["!!!ADMIN: >PERM< %2 out of %1 is set to % 3!", name player, _perm_name, _level]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};
