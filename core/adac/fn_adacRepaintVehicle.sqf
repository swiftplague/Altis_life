...
{
	hint "You are not a member of ADAC!";
...
if(_index == -1) exitWith {hint "no color japnese.";};
...
if((_veh isKindOf "Car") OR (_veh isKindOf "Ship") OR (_veh isKindOf "Air")) then
{
	if(([false,"uitem_adac_paint",_color_paint_buckets] call life_fnc_handleInv)) then
	{
		...
		_upp = format["Medic %1",_displayName];
		...
		if(player != vehicle player) exitWith {titleText["you got to get out of the vehicle, to can help paint it to ok!","PLAIN"];};
	...
	{
		hint "You not enough paint around the vehicle to colours has.";
