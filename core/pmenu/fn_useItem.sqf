...
switch (true) do
{
	...
	case (_item == "redgull"):
	{
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
			...
			[] spawn
			{
				...
				titleText["You are inexhaustible for 3 minutes!","PLAIN"];
	...
	{
		if(!isNull life_spikestrip) exitWith {hint "you already have a Spike Strip in active use."};
		if(([false,_item,1] call life_fnc_handleInv)) then
		{
	...
	{
		if(vehicle player != player) exitWith {hint "you can't fill the vehicle not from the inside!!"};
	...
	default
	{
		hint "The item is not available.";
	};
};
