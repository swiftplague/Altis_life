...
if(!_canown) exitWith { hint "The flag is taken up just!";};
...
if(!_canown) exitWith { hint "Cool down the flag is still not expired!";};

//own the pole 
 [[0,format["** EVENT ** %1 is just a flag!", name player]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
...
switch(true) do 
{
	case (_tazed):
	{
		hint "You were tazed and got no points!";
		_cooldown = 10;
	};
