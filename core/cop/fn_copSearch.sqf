...
if((count _invs) + (count _weapons) > 0) then
{
	...
	[[0,format["%1 has items in the value of $ this %2.",name _civ,[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
...
if(_robber) then
{
	[[0,format["%1 has been identified as bank robber!",name _civ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};