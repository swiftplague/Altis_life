...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			titleText["[AUTO] you're unleashed.","PLAIN"];
		};
	};
};

titleText[format["You were captivated by % 1.",name _cop],"PLAIN"];
...
{
	titleText["you were released. [Press V to you for free.","PLAIN"];
};