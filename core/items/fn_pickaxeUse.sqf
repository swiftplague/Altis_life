...
if(_mine == "") exitWith 
{
	if(!_silent) then
	{
		...
		hint "You are not close to a mine!"
	};
};
if(vehicle player != player) exitWith {hint "you can not build from within by your car!";};
...
if(([true,_mine,_diff] call life_fnc_handleInv)) then
{
	...
	titleText[format["You built from %2% 1.",_itemName,_diff],"PLAIN"];
