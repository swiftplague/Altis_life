...
if(_isVehicle && _curTarget in life_vehicles) exitWith {hint "the vehicle is hint in your Keychain Access."};
...
if(!isNil "_badDistance") exitWith {titleText["you got you too far away from the target.","PLAIN"]; life_action_inUse = false;};
...
if(!_isVehicle) then {
	...
	if(_dice < 15) then {
		...
		titleText["the vehicle is now in your Keychain Access.","PLAIN"];
		...
		[[0,format["%1 tried to break up a car",name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		titleText["Dietrich is broken.","PLAIN"];
	};
};