...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hint format["%1 %2 gave you but you can hold only %3 so %4 have been returned.",name _from,_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hint format["%1 %2 has given you %3",name _from,_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
