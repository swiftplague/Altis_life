...
{
	hint "The House heard you don't!";
...
{
	hint "Someone else already accesses on the House!";
};
...
if( [_exdata, "owned"] call life_fnc_houseExtractDataVal ) then //If owned => Player is owner! NOT, disable if only access
{
	...
	if(!_buyable) then
	{
		(_display displayCtrl 1102) ctrlSetStructuredText parseText "<t size='2' align='center'>Not for sale</t>";
	}
	else
	{
	...
	if(_donator) then
	{
		...
		systemChat "House is a donor's House. If you are a donor, you can use advanced features!";
	}
	else
	{
