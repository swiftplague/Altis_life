...
if(life_session_tries > 3) exitWith {cutText["Connection to the server cannot be made. You have reached the maximum number of 3 attempts. Connect new please! "," BLACK FADED"];0 cutFadeOut 9999999;};
cutText["Get information from Server...","BLACK FADED"];
...
switch (playerSide) do
{
	...
	case civilian:
	{
		...
		//Initialize inventory stuff ##43
		life_inventory = (_session select 9);					
		life_inventory = call compile format["%1", life_inventory];
...
if(_isbanned) exitWith 
{
	...
	civ_gear = [];
	cop_gear = [];
	[[0,format["DBBAN >>> Wanted to join the banned player '%1' <<<", name player]],"life_fnc_broadcast" /*,west,FALSE*/] call life_fnc_MP;
	cutText["You have been banned from this server! Complaints TS: 198.204.226.98","BLACK FADED"];0 cutFadeOut 9999999;
};
...
cutText["Get information from the server and test it, you're almost done.","BLACK FADED"];
0 cutFadeOut 9999999;
